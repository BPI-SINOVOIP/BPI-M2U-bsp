...
#include <../drivers/soc/allwinner/pm_legacy/mem_hwspinlock.h>
/*
...
uint32_t sunxi_do_fast_smc(uint32_t arg0, uint32_t arg1,
		uint32_t arg2, uint32_t arg3)
{
	struct smc_param param;
	int flag;

	/*printk(KERN_ERR "cpu%d enter cmd=0x%x\n", smp_processor_id(), arg1);*/
...
/*printk(KERN_ERR "cpu%d exit cmd=0x%x\n", smp_processor_id(), arg1);*/
...
static u32 sunxi_sec_read_reg(void __iomem *reg)
{
	...
	phys_addr = (u32)reg > (u32)IO_ADDRESS(0) ?
		    ((u32)reg - (u32)IO_ADDRESS(0)) : (u32)reg;

	value = sunxi_do_fast_smc(TEE_SMC_PLAFORM_OPERATION, TE_SMC_READ_REG,
			phys_addr, 0);
...
static u32 sunxi_sec_write_reg(u32 value, void __iomem *reg)
{
	...
	ret = sunxi_do_fast_smc(TEE_SMC_PLAFORM_OPERATION, TE_SMC_WRITE_REG,
			phys_addr, value);
...
{
	return sunxi_do_fast_smc(arg0, arg1, arg2, arg3);
}

static u32 sunxi_load_arisc(void *image, u32 image_size, void *para,
		u32 para_size, u32 para_offset)
{
	...
	/* do smc call */
	ret = sunxi_do_fast_smc(TEE_SMC_PLAFORM_OPERATION,
			TE_SMC_LOAD_ARISC, (u32)virt_to_phys(param), 0);
...
{
	return sunxi_do_fast_smc(TEE_SMC_PLAFORM_OPERATION,
			TE_SMC_SET_SMP_BOOTENTRY, (u32)entry, 0);
}
 ...
 */
static u32 sunxi_sec_set_standby_status(u32 command_type, u32 setting_type,
		u32 addr, u32 val)
{
	u32 ret_val = 0;
	u32 phys_addr;

	phys_addr = (u32)addr > (u32)IO_ADDRESS(0) ?
		    ((u32)addr - (u32)IO_ADDRESS(0)) : (u32)addr;
	switch (setting_type) {
	case TE_SMC_STANDBY_STATUS_CLEAR:
			ret_val = sunxi_do_fast_smc(TEE_SMC_PLAFORM_OPERATION,
					TE_SMC_READ_REG, phys_addr, 0);
#ifndef CONFIG_ARCH_SUN8IW7P1
			if (!pm_hwspin_lock_timeout(MEM_RTC_REG_HWSPINLOCK,
						20000)) {
#endif
				ret_val = sunxi_do_fast_smc(
...
static u32 sunxi_sec_get_cp15_status(void *addr)
{
	...
	phys_addr = (u32)addr > (u32)IO_ADDRESS(0) ?
		    ((u32)addr - (u32)IO_ADDRESS(0)) : (u32)addr;
	ret_val = sunxi_do_fast_smc(TEE_SMC_PLAFORM_OPERATION,
			TE_SMC_CP15_STATUS_GET, phys_addr, 0);
...
static const struct firmware_secure_ops sunxi_firmware_secure_ops = {
...
{
	register_firmware_secure_ops(&sunxi_firmware_secure_ops);
}
