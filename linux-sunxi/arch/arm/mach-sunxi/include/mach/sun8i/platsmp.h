...
static inline void enable_cpu(int cpu_nr)
{
	...
	sunxi_smc_writel(0x00,
			 SUNXI_R_PRCM_VBASE + SUNXI_CPU_PWR_CLAMP(cluster,
								  cpu));
	while (0x00 !=
	       sunxi_smc_readl(SUNXI_R_PRCM_VBASE +
			       SUNXI_CPU_PWR_CLAMP(cluster, cpu))) {
		;
	}
	mdelay(5);
	...
	value =
	    sunxi_smc_readl(SUNXI_R_PRCM_VBASE +
			    SUNXI_CLUSTER_PWROFF_GATING(cluster));
	value &= (~(0x1 << cpu));
	sunxi_smc_writel(value,
			 SUNXI_R_PRCM_VBASE +
			 SUNXI_CLUSTER_PWROFF_GATING(cluster));
	mdelay(2);
	...
	/* step3: clear reset */
	value =
	    sunxi_smc_readl(SUNXI_R_CPUCFG_VBASE +
			    SUNXI_CLUSTER_PWRON_RESET(cluster));
	value |= (1 << cpu);
	sunxi_smc_writel(value,
			 SUNXI_R_CPUCFG_VBASE +
			 SUNXI_CLUSTER_PWRON_RESET(cluster));

	/* step4: core reset */
	value =
	    sunxi_smc_readl(SUNXI_CPUXCFG_VBASE + SUNXI_CPU_RST_CTRL(cluster));
	value |= (1 << cpu);
	sunxi_smc_writel(value,
			 SUNXI_CPUXCFG_VBASE + SUNXI_CPU_RST_CTRL(cluster));
...
static inline void enable_cpu(int cpu)
{
	...
	sunxi_smc_writel(0,
			 (void *)(SUNXI_R_CPUCFG_VBASE + CPUX_RESET_CTL(cpu)));
	/* L1RSTDISABLE hold low */
	pwr_reg =
	    sunxi_smc_readl((void *)(SUNXI_R_CPUCFG_VBASE +
				     SUNXI_CPUCFG_GENCTL));
	pwr_reg &= ~(1 << cpu);
	sunxi_smc_writel(pwr_reg,
			 (void *)(SUNXI_R_CPUCFG_VBASE + SUNXI_CPUCFG_GENCTL));

#ifdef CONFIG_ARCH_SUN8IW1
	/* step2: release power clamp */
	/* write bit3, bit4 to 0 */
	sunxi_smc_writel(0xe7,
			 (void *)(SUNXI_R_PRCM_VBASE +
				  SUNXI_CPUX_PWR_CLAMP(cpu)));
	while ((0xe7) !=
	       sunxi_smc_readl((void *)(SUNXI_R_CPUCFG_VBASE +
					SUNXI_CPUX_PWR_CLAMP_STATUS(cpu))))
		;
	...
	pwr_reg =
	    sunxi_smc_readl((void *)(SUNXI_R_PRCM_VBASE +
				     SUNXI_CPU_PWROFF_REG));
	pwr_reg &= ~(0x00000001 << cpu);
	sunxi_smc_writel(pwr_reg,
			 (void *)(SUNXI_R_PRCM_VBASE + SUNXI_CPU_PWROFF_REG));
	mdelay(1);
...
static inline void disable_cpu(int cpu)
{
	...
	pwr_reg =
	    sunxi_smc_readl(IO_ADDRESS(SUNXI_R_PRCM_PBASE) +
			    SUNXI_CPU_PWROFF_REG);
	pwr_reg |= (1 << cpu);
	sunxi_smc_writel(pwr_reg,
			 IO_ADDRESS(SUNXI_R_PRCM_PBASE) + SUNXI_CPU_PWROFF_REG);
	usleep_range(1000, 1000);

#ifdef CONFIG_ARCH_SUN8IW1
	/* step10: active the power output clamp */
	sunxi_smc_writel(0xff,
			 IO_ADDRESS(SUNXI_R_PRCM_PBASE) +
			 SUNXI_CPUX_PWR_CLAMP(cpu));
#endif
}
...
{
#ifdef CONFIG_ARCH_SUN8IW6P1
	sunxi_smc_writel((u32) entry,
			 (void *)(SUNXI_R_CPUCFG_VBASE + PRIVATE_REG0));
#else
	writel((u32) entry,
...
#ifdef CONFIG_ARCH_SUN8IW6P1
	return (void *)
	    sunxi_smc_readl((void *)(SUNXI_R_CPUCFG_VBASE + PRIVATE_REG0));
#else
