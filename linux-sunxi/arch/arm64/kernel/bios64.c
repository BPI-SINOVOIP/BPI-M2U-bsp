/*
 *  linux/arch/arm64/kernel/bios64.c
 *
...
static void pcibios_bus_report_status(struct pci_bus *bus, u_int status_mask, int warn)
{
	...
	list_for_each_entry(dev, &bus->devices, bus_list) {
		...
		if (warn)
			dev_info(&bus->dev, "(%s: %04X) ", pci_name(dev), status);
	}
...
void pcibios_fixup_bus(struct pci_bus *bus)
{
	 ...
	 */
	dev_info(&bus->dev, "PCI: bus%d: Fast back to back transfers %sabled\n",
...
int pcibios_enable_device(struct pci_dev *dev, int mask)
{
	...
	for (idx = 0; idx < 6; idx++) {
		...
		r = dev->resource + idx;
		if (!r->start && r->end) {
			pr_debug("PCI: Device %s not available because"
