...
__s32 NAND_AllocMemoryForDMADescs(__u32 *cpu_addr, __u32 *phy_addr)
{
	...
	p = (void *)dma_alloc_coherent(ndfc_dev, DMA_CONFIG_SIZE,
...
{
	dma_free_coherent(ndfc_dev, DMA_CONFIG_SIZE, (void *)((__u64)(*cpu_addr)), (dma_addr_t)(*phy_addr));
...
int NAND_ReleaseVoltage(void)
{
	...
	if (!IS_ERR(regu1)) {
		...
		if (ret)
			NAND_Print("nand:regulator disable fail ret is %x\n", ret);
		if (IS_ERR(regu1))
			NAND_Print("nand:some err happen,fail to disable regulator vcc-nand!");
	...
	if (!IS_ERR(regu2)) {
		...
		if (ret)
			NAND_Print("nand:regulator disable fail ret is %x\n", ret);
		if (IS_ERR(regu2))
			NAND_Print("nand:some err happen,fail to disable regulator vcc-io!");
...
void Dump_Ccmu_Reg_Show(void)
{
	...
	NAND_Print(KERN_EMERG "Reg 0x01c20080: 0x%x\n",
