...
typedef struct {
	...
	unsigned int ChipCnt; /*the count of the total nand flash
	chips are currently connecting on the CE pin*/
	unsigned int ChipConnectInfo;	/*chip connect information,
	bit == 1 means there is a chip connecting on the CE pin*/
	unsigned int RbCnt;
	unsigned int RbConnectInfo;	/*the connect  information
	of the all rb  chips are connected*/
	unsigned int RbConnectMode;	/*the rb connect  mode*/
	unsigned int BankCntPerChip;	/*the count of the banks in
	one nand chip, multiple banks can support Inter-Leave*/
	unsigned int DieCntPerChip;	/*the count of the dies in
	one nand chip, block management is based on Die*/
	unsigned int PlaneCntPerDie;	/*the count of planes in one
	die, multiple planes can support multi-plane operation*/
	unsigned int SectorCntPerPage;	/*the count of sectors in one
	single physic page, one sector is 0.5k*/
	unsigned int PageCntPerPhyBlk;	/*the count of physic pages
	in one physic block*/
	unsigned int BlkCntPerDie;	/*the count of the physic blocks
	in one die, include valid block and invalid block*/
	unsigned int OperationOpt;	/*the mask of the operation types
	which current nand flash can support support*/
	unsigned int FrequencePar;	/*the parameter of the hardware
	access clock, based on 'MHz'*/
	unsigned int EccMode;	/*the Ecc Mode for the nand flash chip,
	0: bch-16, 1:bch-28, 2:bch_32*/
	unsigned char NandChipId[8];	/*the nand chip id of current
	connecting nand chip*/
	unsigned int ValidBlkRatio;	/*the ratio of the valid physical
	blocks, based on 1024*/
...
typedef struct _boot0_private_head_t {
	...
	unsigned int dram_para[32];	/* DRAM patameters for initialising
	dram. Original values is arbitrary*/
	int uart_port;
...
typedef struct standard_Boot_file_head {
	...
	unsigned char magic[8];	/* ="eGON.BT0" or "eGON.BT1",
	not C-style string*/
