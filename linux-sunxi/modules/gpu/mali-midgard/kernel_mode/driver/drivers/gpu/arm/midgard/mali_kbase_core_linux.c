...
extern struct attribute_group scene_ctrl_attribute_group;
...
static int kbase_platform_device_probe(struct platform_device *pdev)
{
	...
	if (err) {
		dev_err(&pdev->dev, "Sunxi SysFS group creation failed\n");
		kbase_platform_device_remove(pdev);
		return err;
	}
	...
	gpu_thermal_cool_register(thermal_cool_callback);
#endif
