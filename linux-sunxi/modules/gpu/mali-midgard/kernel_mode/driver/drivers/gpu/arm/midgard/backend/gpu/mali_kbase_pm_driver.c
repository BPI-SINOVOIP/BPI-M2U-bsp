...
void kbase_pm_check_transitions_sync(struct kbase_device *kbdev)
{
	...
	/* JetCui:fix the power don't changed bug and make a sure.
	  * and fence time out 3000ms, you designed 5s, so modify it.
	  * and give (hardware and soft) 3 times chance.
	  */
retry:
	...
	ret = wait_event_timeout(kbdev->pm.backend.gpu_in_desired_state_wait,
				kbdev->pm.backend.gpu_in_desired_state, msecs_to_jiffies(timeout));
	...
	if (ret <= 0) {
		...
		dev_err(kbdev->dev, "\tShader=%08x%08x active:%08x%08x\n",
				...
				kbase_reg_read(kbdev,
					...
					NULL));
		dev_err(kbdev->dev, "\tTiler =%08x%08x active:%08x%08x\n",
				kbase_reg_read(kbdev,
