...
static int sndi2s1_set_dai_clkdiv(struct snd_soc_dai *codec_dai,
	int div_id, int div)
{
	return 0;
}
...
static int __init sndi2s1_codec_probe(struct platform_device *pdev)
{
	return snd_soc_register_codec(&pdev->dev,
		&soc_codec_dev_sndi2s1, &sndi2s1_dai, 1);
}
...
static int __init sndi2s1_codec_init(void)
{
	...
	err = platform_device_register(&sndi2s1_codec_device);
	if (err < 0)
		return err;

	err = platform_driver_register(&sndi2s1_codec_driver);
	if (err < 0)
		return err;
