			...
			int stream, int event)
{
	...
	struct snd_soc_dai *codec_dai = card->rtd[event_bind_id].codec_dai;
	int opt_open;
...
static int sunxi_ahub_card_init(struct snd_soc_pcm_runtime *rtd)
{
	...
	snd_soc_dapm_disable_pin(&codec->dapm, "DAM0IN");
	snd_soc_dapm_disable_pin(&codec->dapm, "DAM1IN");
	snd_soc_dapm_disable_pin(&codec->dapm, "DAM0OUT");
	snd_soc_dapm_disable_pin(&codec->dapm, "DAM1OUT");
