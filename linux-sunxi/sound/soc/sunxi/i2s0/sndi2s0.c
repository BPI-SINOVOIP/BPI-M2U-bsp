...
static int sndi2s0_set_dai_clkdiv(struct snd_soc_dai *codec_dai,
	int div_id, int div)
{
	return 0;
}
...
static int __init sndi2s0_codec_probe(struct platform_device *pdev)
{
	return snd_soc_register_codec(&pdev->dev,
		&soc_codec_dev_sndi2s0, &sndi2s0_dai, 1);
}
...
static int __init sndi2s0_codec_init(void)
{
	...
	err = platform_device_register(&sndi2s0_codec_device);
	if (err < 0)
		return err;

	err = platform_driver_register(&sndi2s0_codec_driver);
	if (err < 0)
		return err;
