	...
	struct snd_pcm_hw_params *params)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		...
		slave_config.slave_id = sunxi_slave_id(DRQDST_DAUDIO_0_TX,
			DRQSRC_SDRAM);
...
static int __init sunxi_soc_platform_i2s0_init(void)
{
	...
	err = platform_device_register(&sunxi_i2s0_pcm_device);
	if (err < 0)
		return err;

	err = platform_driver_register(&sunxi_i2s0_pcm_driver);
	if (err < 0)
		return err;
	return 0;
}
