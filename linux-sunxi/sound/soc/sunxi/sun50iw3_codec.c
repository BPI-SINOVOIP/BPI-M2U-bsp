			...
			struct snd_kcontrol *k,	int event)
{
	...
	switch (event) {
	case SND_SOC_DAPM_POST_PMU:
		snd_soc_update_bits(codec, LINEOUT_CTRL1,
				    (0x3 << LINEOUT_RIGHT_EN),
				    (0x3 << LINEOUT_RIGHT_EN));
		break;
	case SND_SOC_DAPM_PRE_PMD:
		snd_soc_update_bits(codec, LINEOUT_CTRL1,
				    (0x1 << LINEOUT_RIGHT_EN),
				    (0x0 << LINEOUT_RIGHT_EN));
		break;
	}
...
static const struct snd_kcontrol_new sunxi_codec_controls[] = {
	...
	SOC_SINGLE_TLV("lineout volume", LINEOUT_CTRL1, LINEOUT_VOL, 0x1f, 0,
		       lineout_vol_tlv),
	SOC_SINGLE_TLV("speaker volume", SPKOUT_CTRL0, SPKOUT_VOL, 0x1f, 0,
		       speaker_vol_tlv),
	...
	SOC_SINGLE_TLV("Phoneout gain control", PHONEOUT_CTRL, PHONEOUTGAIN,
		       0x7, 0, phoneout_vol_tlv),
};
...
static const struct soc_enum ac_rspks_func_enum =
	SOC_ENUM_SINGLE(SPKOUT_CTRL1, RIGHT_SPK_SRC_SEL, 2, ac_rspks_func_sel);
...
static const char * const mic2src_text[] = {"MIC2", "MIC3"};
...
static const struct snd_soc_dapm_widget ac_dapm_widgets[] = {
	...
	SND_SOC_DAPM_MUX_E("SPK_R Mux", SND_SOC_NOPM, 0, 0,
			&ac_rspks_func_controls, sunxi_spkl_event,
			SND_SOC_DAPM_PRE_PMU|SND_SOC_DAPM_POST_PMD),
	SND_SOC_DAPM_MUX_E("SPK_L Mux", SND_SOC_NOPM, 0, 0,
			&ac_lspks_func_controls, sunxi_spkr_event,
			SND_SOC_DAPM_PRE_PMU|SND_SOC_DAPM_POST_PMD),
...
static int codec_resume(struct snd_soc_codec *codec)
{
	...
	/*we need 30ms to wait avcc stable,other write analog register will error*/
	mdelay(30);
	codec_init(sunxi_internal_codec);
	if (spk_gpio.cfg) {
