...
#if (0)
#define SWITCH_DBG(format, args...)  pr_info("[SWITCH] "format, ##args)
#else
#define SWITCH_DBG(...)
#endif
...
static int headphone_mute_used;
...
/*
* CIRCLE_COUNT == 0,
* check the earphone state three times(audio_hmic_irq:one time and
* earphone_switch_timer_poll:two times)
* CIRCLE_COUNT == 1,
* check the earphone state four times(audio_hmic_irq:one time and
* earphone_switch_timer_poll:three times)
* CIRCLE_COUNT == 2, check the earphone state five times(audio_hmic_irq:
* one time and earphone_switch_timer_poll:four times)
*/
...
static struct workqueue_struct *resume_switch_work_queue;
	...
	if (!headphone_direct_used) {
		if (((tmp >= 0xb) &&
			...
			if ((switch_data->check_three_count > CIRCLE_COUNT) &&
				(switch_data->state != 2)) {
				/*three sections earphone has plun in*/
				...
				hmic_wr_control(SUNXI_HMIC_CTL,
					0x1, HMIC_DIRQ, 0x0); /*23*/
			}
			/*check again to reduce the disturb from
			 * earphone plug in unstable
			 */
			...
			if ((switch_data->check_four_count > CIRCLE_COUNT) &&
				(switch_data->state != 1)) {
				/*four sections earphone has plun in*/
				switch_data->mode = FOUR_HEADPHONE_PLUGIN;
				switch_data->state = 1;
				schedule_work(&switch_data->work);
				SWITCH_DBG("%s,line:%d\n", __func__, __LINE__);
			...
			(switch_data->state == 1) &&
			(switch_data->reset_flag == 0)) {
			SWITCH_DBG("%s,line:%d,reset_flag:%d,tmp:%x\n",
				__func__, __LINE__,
				switch_data->reset_flag, tmp);
			/*spin_lock(&switch_data->lock);*/
			if ((tmp >= 0x10) && (switch_data->reset_flag == 0)) {
				SWITCH_DBG("H_SCH\n");
				/*
				schedule_work(&switch_data->hook_work);
				queue_work(switch_hook_queue,
				&switch_data->hook_work);
				switch_data->check_hook_count = 0;
				*/
				switch_data->mode = HOOK_CHCCK_DOWN;
				hmic_wr_control(SUNXI_HMIC_CTL,
					0x1, HMIC_DIRQ, 0x1);/*23*/
			} else if ((tmp == 0xf) &&
				(switch_data->reset_flag == 0)) {
				input_report_key(switch_data->key,
					KEY_VOLUMEUP, 1);
				input_sync(switch_data->key);
				input_report_key(switch_data->key,
					KEY_VOLUMEUP, 0);
				input_sync(switch_data->key);
				/*hmic_wr_control(SUN6I_HMIC_CTL,
				 * 0x1, HMIC_DIRQ, 0x1);
				 */
			} else if ((tmp == 0xe) &&
				(switch_data->reset_flag == 0)) {
			...
			if ((switch_data->reset_flag == 0) &&
				(switch_data->check_hook_count > 1)) {
				SWITCH_DBG("vol%s,l:%d,reset_flag:%d,tmp:%x\n",
					__func__, __LINE__,
					switch_data->reset_flag, tmp);
				/*hook down*/
				input_report_key(switch_data->key,
			...
			SWITCH_DBG("state:%d,reset_flag:%d, tmp:%x\n",
				switch_data->state,
				switch_data->reset_flag, tmp);
		...
		if (tmp > 1) {
			...
			if (switch_data->check_three_count >
				(CIRCLE_COUNT-1)) {
				/*three sections earphone has plun in*/
				switch_data->state = 2;
				schedule_work(&switch_data->work);
			...
			/*if the irq is hmic earphone pull out,
			 *when the irq coming, clean the pending bit
			 */
		}
...
static irqreturn_t audio_hmic_irq(int irq, void *dev_id)
{
	...
	if (!headphone_direct_used) {
		if ((switch_data->state == -1) || (tmp >= 1)) {
			SWITCH_DBG("h-1\n");
			/*�ɼ����ݣ��˲��ж�*/
			if (switch_data->check_debounce_count <=
				HEADSET_DEBOUNCE) {
				debounce_val[switch_data->check_debounce_count]
				    = tmp;
			...
			if ((debounce_val[0] == debounce_val[1]) &&
				(debounce_val[1] == debounce_val[2]) &&
				(debounce_val[2] == debounce_val[3])) {
				debounce_ave = (int)((debounce_val[0] +
					debounce_val[1] + debounce_val[2] +
					debounce_val[3])/4);
				...
				if (debounce_ave >= 1) {
					...
					switch_data->timer.function =
						earphone_switch_timer_poll;
					switch_data->timer.data =
				...
				SWITCH_DBG("switch_data->state:%d\n",
				switch_data->state);
				SWITCH_DBG("debounce_val[0-3]=[%d,%d,%d,%d]\n",
				debounce_val[0], debounce_val[1],
				debounce_val[2], debounce_val[3]);
				if ((debounce_val[0] == 0) ||
					(debounce_val[1] == 0) ||
					(debounce_val[2] == 0) ||
...
{
	/*audio codec hardware bug.
	 *the HBIASADCEN bit must be enable in init
	 */
...
static int gpio_switch_probe(struct platform_device *pdev)
{
	...
	ret = of_property_read_u32(np, "headphone_mute_used", &temp_val);
	if (ret < 0) {
		dev_warn(&pdev->dev,
			"headphone_mute_used missing or invalid\n");
		headphone_mute_used = 0;
	} else {
		headphone_mute_used = temp_val;
		dev_dbg(&pdev->dev, "headphone_mute_used:%d\n", temp_val);
	}

	ret = of_property_read_u32(np, "headphone_direct_used", &temp_val);
	if (ret < 0) {
		dev_warn(&pdev->dev,
			"headphone_direct_used missing or invalid\n");
	...
	/*0xf should be get from hw_debug 28*/
	hmic_wr_control(SUNXI_HMIC_CTL, 0xf, HMIC_M, 0x0);
	/*0xf should be get from hw_debug 24 0xf*/
	hmic_wr_control(SUNXI_HMIC_CTL, 0xf, HMIC_N, 0x0);
	if (!headphone_direct_used)
		hmic_wr_control(SUNXI_HMIC_CTL, 0x1, HMIC_DIRQ, 0x1);/*23*/

	hmic_wr_control(SUNXI_HMIC_CTL,
		0x1, HMIC_EARPHONE_OUT_IRQ_EN, 0x1); /*20*/
	hmic_wr_control(SUNXI_HMIC_CTL,
		0x1, HMIC_EARPHONE_IN_IRQ_EN, 0x1); /*19*/
	hmic_wr_control(SUNXI_HMIC_CTL,
		0x1, HMIC_KEY_UP_IRQ_EN, 0x1); /*18*/
	hmic_wr_control(SUNXI_HMIC_CTL,
		0x1, HMIC_KEY_DOWN_IRQ_EN, 0x1); /*17*/
	hmic_wr_control(SUNXI_HMIC_CTL,
		0x1, HMIC_DATA_IRQ_EN, 0x1); /*16*/
	hmic_wr_control(SUNXI_HMIC_CTL,
		0x3, HMIC_DS_SAMP, 0x0); /*14 */
	/*0xf should be get from hw_debug 8*/
	hmic_wr_control(SUNXI_HMIC_CTL,
		0x1f, HMIC_TH2_KEY, 0x0);
	/*0x1 should be get from hw_debug 0*/
	hmic_wr_control(SUNXI_HMIC_CTL,
		0x1f, HMIC_TH1_EARPHONE, 0x1);
	...
	if (!switch_data->key) {
		pr_err(KERN_ERR "gpio_switch_probe err\n");
		ret = -ENOMEM;
	...
	if (ret) {
		pr_err(KERN_ERR "input_register_device failed\n");
	...
	#ifdef CONFIG_ARCH_SUN8IW5
	ret = request_irq(78, audio_hmic_irq,
		0, "audio_hmic_irq", switch_data);
	#else
	ret = request_irq(SUNXI_IRQ_CODEC, audio_hmic_irq,
		0, "audio_hmic_irq", switch_data);
	#endif
	if (ret < 0) {
	...
	resume_switch_work_queue =
		create_singlethread_workqueue("switch_resume");
	if (resume_switch_work_queue == NULL) {
		pr_err("[switch_headset]create workqueue for codec failed!\n");
		ret = -ENOMEM;
	...
	if (headphone_mute_used) {
		...
		/*request gpio*/
		req_mute_status = gpio_request(mute_gpio, NULL);
...
static int switch_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	/*
	*Need more than 450ms to ensure the pa can close normal after
	*set HPPAEN to 0. Maybe cause abnormal power consumption without
	*enough delay.so msleep 500 currently.
	*/
	msleep(500);
...
static int switch_resume(struct platform_device *pdev)
{
	...
	if (check_scene_locked(SCENE_TALKING_STANDBY) != 0) {
		/*0xf should be get from hw_debug 28*/
		hmic_wr_control(SUNXI_HMIC_CTL, 0xf, HMIC_M, 0x0);
		/*0xf should be get from hw_debug 24*/
		hmic_wr_control(SUNXI_HMIC_CTL, 0xf, HMIC_N, 0x0);
		hmic_wr_control(SUNXI_HMIC_CTL,
			0x1, HMIC_EARPHONE_OUT_IRQ_EN, 0x1); /*20*/
		hmic_wr_control(SUNXI_HMIC_CTL,
	...
	if (switch_data != NULL) {
		queue_work(resume_switch_work_queue,
			&switch_data->resume_work);
	}
	return 0;
}
