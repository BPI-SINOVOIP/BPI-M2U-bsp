...
#define SENSOR_NAME_DEFAULT           "ds1339"
#define SENSOR_I2C_ADDR               0x68

static struct ds1307_platform_data ds1307_platform_data = {
	/* No diode, 4k resistor */
	.trickle_charger_setup = 0x7,
};

static struct i2c_board_info ds1307_board_info={
    .type = SENSOR_NAME_DEFAULT, 
    .addr = SENSOR_I2C_ADDR,
    .platform_data = &ds1307_platform_data,
};

static struct i2c_client *ds1307_client;
static const unsigned short normal_i2c[] = { SENSOR_I2C_ADDR, I2C_CLIENT_END };

static int adapter_id = -1;
static const char *rtc_type;
...
static int ds1307_detect(struct i2c_client *client,
			  struct i2c_board_info *info)
{
	...
	/* if rtc_type not defined, driver initial failed, 
	 * you can write correct name and slave_addr to
	 * /sys/class/i2c-adapter/i2c-X/new_device in 
	 * userspace to debug */
	if(rtc_type != NULL){
		strlcpy(info->type, rtc_type, I2C_NAME_SIZE);
	}else {
		pr_err("[BPI-RTC] %s, rtc_type not defined\n", __func__);
		return -1;
	}
...
static int __init ds1307_init(void)
{
	...
	node = of_find_node_by_type(NULL, "rtc_external");
	if(!node) {
		pr_err("[BPI-RTC] can't get device node\n");
		detect = 1;
	}

	if (of_property_read_u32(node, "twi_id", &adapter_id)){
		pr_err("[BPI-RTC] can't get twi_id\n");
		detect = 1;
	}

	if (of_property_read_string(node, "rtc_type", &rtc_type)){
		pr_err("[BPI-RTC] can't get rtc_type\n");
		detect = 1;
	}

	/* if all defined, register device, 
	 * otherwise detect and detect way also need rtc_type defined 
	 */
	if(!detect) {
		pr_info("[BPI-RTC] direct register device, no detect\n");
		
		/* not detect */
		ds1307_driver.class = 0;
		ds1307_driver.detect = NULL;
		ds1307_driver.address_list = NULL;
		
		i2c_adapter = i2c_get_adapter(adapter_id);
		ds1307_client = i2c_new_device(i2c_adapter, &ds1307_board_info);
