...
static int axp_powerkey_probe(struct platform_device *pdev)
{
	...
	if (pdev->dev.of_node) {
		...
		ret = axp_powerkey_dt_parse(pdev->dev.of_node, &axp22_config);
		if (ret) {
			pr_err("%s parse device tree err\n", __func__);
			return -EINVAL;
		}
	} else {
		pr_err("axp22 powerkey device tree err!\n");
		return -EBUSY;
	}
	...
	axp_regmap_read(map, AXP22_POK_SET, &val);
	val &= 0xfc;
	val |= (axp22_config.pmu_powkey_off_time - 4000) / 2000;
	axp_regmap_write(map, AXP22_POK_SET, val);
...
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("onkey Driver for axp22x PMIC");
