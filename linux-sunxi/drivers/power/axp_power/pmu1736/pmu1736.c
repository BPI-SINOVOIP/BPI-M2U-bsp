...
static int pmu1736_probe(struct platform_device *pdev)
{
	...
	pmu1736_pmu_num = axp_get_pmu_num(pmu1736_match,
				ARRAY_SIZE(pmu1736_match));
	...
	pmu1736 = devm_kzalloc(&pdev->dev, sizeof(*pmu1736), GFP_KERNEL);
	if (!pmu1736)
		return -ENOMEM;

	pmu1736->dev = &pdev->dev;
	...
	ret = axp_mfd_cell_name_init(&pmu1736_platform_ops,
				ARRAY_SIZE(pmu1736_match), pmu1736->pmu_num,
				pmu1736->nr_cells, pmu1736->cells);
	if (ret)
		return ret;

	pmu1736->regmap = axp_regmap_init_arisc_rsb(&pdev->dev,
				RSB_RTSADDR_PMU1736);
