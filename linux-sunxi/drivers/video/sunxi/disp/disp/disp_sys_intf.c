...
*******************************************************************************
*/
int disp_sys_register_irq(u32 IrqNo, u32 Flags,
			  void *Handler, void *pArg, u32 DataSize, u32 Prio)
{
	__inf("%s, irqNo=%d, Handler=0x%p, pArg=0x%p\n",
	      __func__, IrqNo, Handler, pArg);
	return request_irq(IrqNo, (irq_handler_t) Handler,
			   IRQF_DISABLED, "dispaly", pArg);
}
			     ...
			     char *sub_name, int value[], int type)
{
	...
	if (type == 1) {
		...
		ret = of_property_read_u32(node, sub_name, &gpio);
		if (ret < 0) {
			__inf("failed to get %s err=%d\n", sub_name, ret);
			goto exit;
		}
		config.gpio = gpio;
#else
		...
		__inf("%s.%s gpio=%d,mul_sel=%d,data:%d\n",
		      main_name, sub_name, gpio_info->gpio,
		      gpio_info->mul_sel, gpio_info->data);
		ret = type;
	}
...
int disp_sys_gpio_release(int p_handler, s32 if_release_to_default_status)
{
	if (p_handler)
		...
		__wrn("GPIO_Release, hdl is NULL\n");
				...
				u32 direction, const char *gpio_name)
{
	...
	if (p_handler) {
		if (direction) {
			s32 value;
#ifdef CONFIG_IO_EXPAND
			value = gpio_get_value_cansleep(p_handler);
#else
		...
		__wrn("GPIO_DevSetONEPIN_IO_STATUS, hdl is NULL\n");
		ret = -1;
	}
	return ret;
}
...
int disp_sys_gpio_get_value(u32 p_handler, const char *gpio_name)
{
	if (p_handler)
#ifdef CONFIG_IO_EXPAND
		return gpio_get_value_cansleep(p_handler);
#else
		return gpio_get_value(p_handler);
#endif
	__wrn("GPIO_DevREAD_ONEPIN_DATA, hdl is NULL\n");
	return -1;
}
...
int disp_sys_clk_set_parent(const char *id, const char *parent)
{
	...
	if (hckl_parent == NULL || IS_ERR(hckl_parent)) {
		__wrn("Fail to get handle for clock %s.\n", parent);
		clk_put(hclk);
		return -1;
	}
