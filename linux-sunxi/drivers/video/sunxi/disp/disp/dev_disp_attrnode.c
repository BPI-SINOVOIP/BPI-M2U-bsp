	...
	for (screen_id = 0; screen_id < num_screens; screen_id++) {
		...
		if (bsp_disp_get_output_type(screen_id)
			...
			count += sprintf(buf + count,
					 "\tlcd output\tbacklight(%3d)",
					 bsp_disp_lcd_get_bright(screen_id));
			...
			if (bsp_disp_hdmi_get_mode(screen_id) ==
			    DISP_TV_MOD_720P_50HZ) {
				count += sprintf(buf + count, "\t%16s",
						 "720p50hz");
			} else if (BSP_disp_hdmi_get_mode(screen_id) ==
		...
		for (layer_id = 0; layer_id < num_layers; layer_id++) {
			...
			ret = bsp_disp_layer_get_info(screen_id,
						      layer_id, &layer_para);
			enabled = bsp_disp_layer_is_enabled(screen_id,
							    layer_id);
			if (ret == 0) {
				...
				count += sprintf(buf + count,
						 ...
						 (enabled ==
						  1) ? "enable" : "disable");
				count +=
				    sprintf(buf + count, " %4d |", layer_id);
				count +=
				    sprintf(buf + count, " %4d |",
					    layer_para.pipe);
				count +=
				    sprintf(buf + count, " %1d |",
					    layer_para.zorder);
				count +=
				    sprintf(buf + count, " %8s |",
					    (layer_para.fb.
					     pre_multiply) ? "Y" : "N");
				count +=
				    sprintf(buf + count, " %5s(%3d) |",
					    (layer_para.
					     alpha_mode) ? "globl" : "pixel",
					    layer_para.alpha_value);
				count +=
				    sprintf(buf + count, " %8s |",
					    (layer_para.
					     ck_enable) ? "enable" : "disable");
				count +=
				    sprintf(buf + count, " %3d |",
					    layer_para.fb.format);
				count +=
				    sprintf(buf + count, " [%4d,%4d] |",
					    layer_para.fb.size.width,
					    layer_para.fb.size.height);
				count +=
				    sprintf(buf + count, " [%4d,%4d,%4d,%4d] |",
					    layer_para.fb.src_win.x,
					    layer_para.fb.src_win.y,
					    layer_para.fb.src_win.width,
					    layer_para.fb.src_win.height);
				count +=
				    sprintf(buf + count, " [%4d,%4d,%4d,%4d] |",
					    layer_para.screen_win.x,
					    layer_para.screen_win.y,
					    layer_para.screen_win.width,
					    layer_para.screen_win.height);
				count +=
				    sprintf(buf + count, " [%1d%1d,%1d%1d] |",
					    layer_para.fb.b_trd_src,
					    layer_para.fb.trd_mode,
					    layer_para.b_trd_out,
					    layer_para.out_trd_mode);
				count +=
				    sprintf(buf + count, " [%8x,%8x,%8x] |",
					    layer_para.fb.addr[0],
					    layer_para.fb.addr[1],
					    layer_para.fb.addr[2]);
				count += sprintf(buf + count, "\n");
			}
		}
				   ...
				   const char *buf, size_t count)
{
	...
	if ((val > 7)) {
		pr_warn("Invalid value, 0~7 is expected!\n");
	} else {
		fb_draw_colorbar((__u32 __force) g_fbi.fbinfo[val]->screen_base,
				 g_fbi.fbinfo[val]->var.xres,
				 g_fbi.fbinfo[val]->var.yres,
				 &(g_fbi.fbinfo[val]->var));
	}
...
static DEVICE_ATTR(runtime_enable, S_IRUGO | S_IWUSR,
	disp_runtime_enable_show, disp_runtime_enable_store);
#endif

static ssize_t disp_suspend_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%s\n", "nothing");
}

static ssize_t disp_suspend_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	int err;
	unsigned long val;
...
static DEVICE_ATTR(suspend, S_IRUGO | S_IWUSR,
	disp_suspend_show, disp_suspend_store);
