...
static struct capture_info_t capture_info;
...
static void capture_work_0(struct work_struct *work)
{
	...
	snprintf(buf, sizeof(buf), "CAPTURE0=BUFFER%d,TIME=%llu",
		 bsp_disp_capture_screen_get_buffer_id(0),
		 ktime_to_ns(capture_info.capture_timestamp[0]));
	envp[0] = buf;
	envp[1] = NULL;
...
static void capture_work_1(struct work_struct *work)
{
	...
	snprintf(buf, sizeof(buf), "CAPTURE1=BUFFER%d,TIME=%llu",
		 bsp_disp_capture_screen_get_buffer_id(1),
		 ktime_to_ns(capture_info.capture_timestamp[1]));
	envp[0] = buf;
	envp[1] = NULL;
...
static ssize_t capture_read(struct file *file,
			    char __user *buf, size_t count, loff_t *ppos)
{
	return 0;
}

static ssize_t capture_write(struct file *file,
...
static long capture_ioctl(struct file *file,
...
static int capture_probe(struct platform_device *pdev)
{
	...
	memset(&capture_info, 0, sizeof(struct capture_info_t));
