...
static s32 disp_mgr_clk_init(struct disp_manager *mgr)
{
	...
	if (mgrp->clk.clk &&
	   (__clk_get_enable_count(mgrp->clk.clk) != 0))
		clk_disable(mgrp->clk.clk);

	if (mgrp->extra_clk &&
	   (__clk_get_enable_count(mgrp->extra_clk) != 0))
...
static s32 disp_mgr_clk_enable(struct disp_manager *mgr)
{
	...
	ret = clk_prepare_enable(mgrp->clk.clk);
	if (ret != 0)
...
static s32 disp_mgr_init(struct disp_manager *mgr)
{
	...
	if ((mgr->p_sw_init_flag != NULL) && (0 != *(mgr->p_sw_init_flag))) {
#if defined(CONFIG_HOMLET_PLATFORM)
		disp_mgr_clk_enable_sw(mgr);
		/* disable irq of imageloadfinish */
		disp_al_manager_init_sw(mgr->channel_id);
	...
	if (request_irq(mgrp->irq_no,
			(irq_handler_t) manager_event_proc,
			IRQF_DISABLED, "dispaly", (void *)mgr->channel_id))
		DE_WRN("request_irq err!");
#if !defined(__LINUX_PLAT__)
	enable_irq(mgrp->irq_no);
#endif
	return 0;
}
...
#if defined(__LINUX_PLAT__)
	disable_irq(mgrp->irq_no);
#endif
	free_irq(mgrp->irq_no, (void *)mgr->channel_id);

	return 0;
}

static s32 disp_mgr_set_back_color(struct disp_manager *mgr,
				   disp_color_info *back_color)
{
	struct disp_manager_private_data *mgrp = disp_mgr_get_priv(mgr);
...
#if defined(__LINUX_PLAT__)
	{
		...
		spin_lock_irqsave(&mgr_data_lock, flags);
#endif
		memcpy(&mgrp->user_info.back_color,
		       back_color, sizeof(disp_color_info));
...
static s32 disp_mgr_get_back_color(struct disp_manager *mgr,
				   disp_color_info *back_color)
{
	struct disp_manager_private_data *mgrp = disp_mgr_get_priv(mgr);
...
#if defined(__LINUX_PLAT__)
	{
		...
		spin_lock_irqsave(&mgr_data_lock, flags);
#endif
		memcpy(back_color, &mgrp->user_info.back_color,
...
static s32 disp_mgr_set_output_type(struct disp_manager *mgr,
...
static s32 disp_mgr_get_output_type(struct disp_manager *mgr,
				    disp_output_type *output_type)
{
	struct disp_manager_private_data *mgrp = disp_mgr_get_priv(mgr);
...
static s32 disp_mgr_apply(struct disp_manager *mgr)
{
...
#if defined(__LINUX_PLAT__)
	{
		...
		if (mgrp->user_info_dirty) {
			memcpy(&mgrp->info, &mgrp->user_info,
			       sizeof(struct disp_manager_info));
...
static s32 disp_mgr_set_screen_size(struct disp_manager *mgr, u32 width,
				    u32 height)
{
	struct disp_manager_private_data *mgrp = disp_mgr_get_priv(mgr);
...
static s32 disp_mgr_get_screen_size(struct disp_manager *mgr, u32 *width,
				    u32 *height)
{
	struct disp_manager_private_data *mgrp = disp_mgr_get_priv(mgr);
...
#if defined(__LINUX_PLAT__)
	{
		...
		spin_lock_irqsave(&mgr_data_lock, flags);
#endif
		disp_al_manager_get_display_size(mgr->channel_id, width,
						 height);
#if defined(__LINUX_PLAT__)
		spin_unlock_irqrestore(&mgr_data_lock, flags);
	}
...
static s32 disp_mgr_add_layer(struct disp_manager *mgr, struct disp_layer *lyr)
{
	...
	if ((mgr == NULL) || (mgrp == NULL) || (lyr == NULL)) {
		DE_WRN("NULL hdl!\n");
		return -1;
	}
...
s32 disp_init_mgr(struct __disp_bsp_init_para *para)
{
	...
	mgrs = kmalloc_array(num_screens,
			     sizeof(struct disp_manager),
			     GFP_KERNEL | __GFP_ZERO);
	if (mgrs == NULL) {
		DE_WRN("malloc memory fail!\n");
		return DIS_FAIL;
	}
	mgr_private = kmalloc_array(num_screens,
				    sizeof(struct disp_manager_private_data),
				    GFP_KERNEL | __GFP_ZERO);
	if (mgr_private == NULL) {
	...
	for (screen_id = 0; screen_id < num_screens; screen_id++) {
		...
		case 0:
			...
			mgrp->reg_base =
			    para->reg_base[DISP_MOD_DE] + SUNXI_DE_BE0_OFFSET;
			mgrp->shadow_protect = para->shadow_protect;
			mgrp->clk.clk = para->mclk[MOD_CLK_DEBE0];
		...
		case 1:
			...
			mgrp->reg_base =
			    para->reg_base[DISP_MOD_DE] + SUNXI_DE_BE1_OFFSET;
			mgrp->shadow_protect = para->shadow_protect;
			mgrp->clk.clk = para->mclk[MOD_CLK_DEBE1];
		...
		case 2:
			...
			mgrp->reg_base =
			    para->reg_base[DISP_MOD_DE] + SUNXI_DE_BE2_OFFSET;
			mgrp->shadow_protect = para->shadow_protect;
			mgrp->clk.clk = para->mclk[MOD_CLK_DEBE2];

			break;
		}
		mgr->p_sw_init_flag = ((1 << screen_id) &
