...
static s32 disp_smcl_sync(struct disp_smcl *smcl)
{
	...
	/* FIXME: display size may be set atfer */
	/* sync(widthout shadow_info_dirty) */
	if (smclp->shadow_info_dirty || smclp->enabled)
		disp_al_smcl_sync(smcl->channel_id);

	if ((smclp->enabled == 0) &&
	    (smclp->clk_close_request == 1) && smclp->shadow_info_dirty) {
...
static s32 disp_smcl_update_regs(struct disp_smcl *smcl)
{
...
#if defined(__LINUX_PLAT__)
	{
		...
		if ((!smcl->manager) || (!smcl->manager->is_enabled) ||
		    (!smcl->manager->is_enabled(smcl->manager)) ||
		    (!smclp->screen_info.info_dirty)) {
#if defined(__LINUX_PLAT__)
			spin_unlock_irqrestore(&smcl_data_lock, flags);
#endif
			return DIS_SUCCESS;
		}
		memcpy(&smcl_info, &smclp->screen_info.info,
		       sizeof(struct disp_smcl_info));
		DE_INF("smcl_update_regs ok, enable=%d\n", smcl_info.enable);
		DE_INF("<%d,%d,%d>,mode=%d, win[%d,%d,%d,%d]",
		       smcl_info.bright,
		       smcl_info.saturation, smcl_info.hue,
		       smcl_info.mode, smcl_info.window.x,
		       smcl_info.window.y, smcl_info.window.width,
		       smcl_info.window.height);
#if defined(__LINUX_PLAT__)
		spin_unlock_irqrestore(&smcl_data_lock, flags);
	}
#endif
	disp_smcl_shadow_protect(smcl, 1);
	disp_al_smcl_set_para(smcl->channel_id, smcl_info.bright,
			      smcl_info.saturation, smcl_info.hue,
			      smcl_info.mode);
...
static s32 disp_smcl_apply(struct disp_smcl *smcl)
{
...
#if defined(__LINUX_PLAT__)
	{
		...
		if (smclp->screen_info.user_info_dirty) {
			memcpy(&smclp->screen_info.info,
			       &smclp->screen_info.user_info,
			       sizeof(struct disp_smcl_info));
...
static s32 disp_smcl_set_window(struct disp_smcl *smcl, disp_window *window)
{
...
#if defined(__LINUX_PLAT__)
	{
		...
		spin_lock_irqsave(&smcl_data_lock, flags);
#endif
		memcpy(&smclp->screen_info.user_info.window,
		       window, sizeof(disp_window));
		smclp->screen_info.user_info_dirty = true;
#if defined(__LINUX_PLAT__)
		spin_unlock_irqrestore(&smcl_data_lock, flags);
	}
...
s32 disp_smcl_set_size(struct disp_smcl *smcl, disp_size *size)
{
...
#if defined(__LINUX_PLAT__)
	{
		...
		spin_lock_irqsave(&smcl_data_lock, flags);
#endif
		memcpy(&smclp->screen_info.user_info.size,
		       size, sizeof(disp_size));
		smclp->screen_info.user_info_dirty = true;
#if defined(__LINUX_PLAT__)
		spin_unlock_irqrestore(&smcl_data_lock, flags);
	}
...
static s32 disp_smcl_set_manager(struct disp_smcl *smcl,
				 struct disp_manager *mgr)
{
	if ((smcl == NULL) || (mgr == NULL)) {
		DE_INF("NULL hdl!\n");
		return -1;
	}
#if defined(__LINUX_PLAT__)
...
s32 disp_init_smcl(struct __disp_bsp_init_para *para)
{
	...
	smcls = kmalloc_array(num_smcls,
			      sizeof(struct disp_smcl),
			      GFP_KERNEL | __GFP_ZERO);
	if (smcls == NULL) {
		DE_WRN("malloc memory fail!\n");
		return DIS_FAIL;
	}
	smcl_private = kmalloc_array(num_smcls,
				     sizeof(struct disp_smcl_private_data),
				     GFP_KERNEL | __GFP_ZERO);
	if (smcl_private == NULL) {
		DE_WRN("malloc memory fail!\n");
		return DIS_FAIL;
	}
