...
static s32 disp_lyr_enable(struct disp_layer *lyr)
{
	...
	if (lyr->manager && lyr->manager->is_enabled &&
	    lyr->manager->is_enabled(lyr->manager)) {
...
static s32 disp_lyr_update_regs(struct disp_layer *lyr)
{
	...
	size = (layer_info.fb.size.width *
		layer_info.screen_win.height * bpp + 7) / 8;
	...
	disp_al_layer_set_alpha_mode(lyr->channel_id,
				     lyr->layer_id, layer_info.alpha_mode,
				     layer_info.alpha_value);
	disp_al_layer_color_key_enable(lyr->channel_id, lyr->layer_id,
				       layer_info.ck_enable);
	disp_al_layer_set_screen_window(lyr->channel_id, lyr->layer_id,
					&layer_info.screen_win);
	disp_al_layer_enable(lyr->channel_id, lyr->layer_id, lyrp->enabled);
	if ((lyr->p_sw_init_flag == NULL) || (0 == *(lyr->p_sw_init_flag))) {
		disp_al_layer_set_extra_info(lyr->channel_id,
					     lyr->layer_id, &layer_info,
					     &lyrp->extra_info);
	} else {
#if defined(CONFIG_HOMLET_PLATFORM)
		disp_al_layer_set_extra_info_sw(lyr->channel_id,
						lyr->layer_id, &layer_info,
						&lyrp->extra_info);
#endif
	}
	...
	disp_al_layer_set_framebuffer(lyr->channel_id,
				      lyr->layer_id, &layer_info.fb);
...
static s32 disp_lyr_apply(struct disp_layer *lyr)
{
...
#if defined(__LINUX_PLAT__)
	{
		...
		if (lyrp->user_info_dirty) {
			memcpy(&lyrp->info, &lyrp->user_info,
...
static s32 disp_lyr_set_info(struct disp_layer *lyr,
			     disp_layer_info *player_info)
{
	struct disp_layer_private_data *lyrp = disp_lyr_get_priv(lyr);
...
static s32 disp_lyr_get_info(struct disp_layer *lyr,
			     disp_layer_info *player_info)
{
	struct disp_layer_private_data *lyrp = disp_lyr_get_priv(lyr);
...
static s32 disp_lyr_set_manager(struct disp_layer *lyr,
...
s32 disp_lyr_deinterlace_cfg(struct disp_layer *lyr)
{
	...
	if (lyrp->extra_info.b_scaler_mode != 0) {
...
s32 disp_init_lyr(struct __disp_bsp_init_para *para)
{
	...
	num_screens = bsp_disp_feat_get_num_screens();
	max_num_layers = bsp_disp_feat_get_num_layers(0);
	lyrs = kmalloc(sizeof(struct disp_layer) * num_screens *
	...
	lyr_private = kmalloc(sizeof(struct disp_layer_private_data) *
			      num_screens * max_num_layers,
	...
	for (screen_id = 0; screen_id < num_screens; screen_id++) {
		...
		for (layer_id = 0; layer_id < num_layers; layer_id++) {
			struct disp_layer *lyr =
			    &lyrs[screen_id * max_num_layers + layer_id];
			struct disp_layer_private_data *lyrp =
			    &lyr_private[screen_id * max_num_layers + layer_id];
