...
static struct __de_format_t scal_input_format_array[] = {
	...
	/*DE_SCAL_UVCOMBINEDMB_128X32 ,DE_SCAL_INYUV411 */
	/*,DE_SCAL_VUVU    ,0 ,}, */
	{DISP_FORMAT_YUV444_P_TILE, DE_SCAL_PLANNARMB,
	 DE_SCAL_INYUV444, 0 /*P3210 */, 0,},
	{DISP_FORMAT_YUV422_P_TILE, DE_SCAL_PLANNARMB,
	 DE_SCAL_INYUV422, 0, 0,},
	{DISP_FORMAT_YUV420_P_TILE, DE_SCAL_PLANNARMB,
	 DE_SCAL_INYUV420, 0, 0,},
	{DISP_FORMAT_YUV411_P_TILE, DE_SCAL_PLANNARMB,
	 DE_SCAL_INYUV411, 0, 0,},
};
...
s32 disp_al_format_to_bpp(disp_pixel_format fmt)
{
	...
	case DISP_FORMAT_YUV444_P_TILE:
		return 24;
	...
	case DISP_FORMAT_YUV422_P_TILE:
		return 16;
	...
	case DISP_FORMAT_YUV420_P_TILE:
	case DISP_FORMAT_YUV411_P_TILE:
		return 12;
...
s32 disp_init_al(struct __disp_bsp_init_para *para)
{
	u32 num_screens;
	u32 i;
	struct __disp_al_mod_list_t *new_mod, *temp;
	struct disp_notifier_block *ptr, *ptr_temp;
...
err_kfree_array:
	list_for_each_entry_safe(ptr, ptr_temp, &al_notifier_list.list, list) {
		list_del(&ptr->list);
		kfree(ptr);
	}
	list_for_each_entry_safe(new_mod, temp, &mod_list.list, list) {
		list_del(&new_mod->list);
		kfree(new_mod);
	}
	kfree(al_private);
err_exit:
...
s32 disp_deinit_al(void)
{
	struct __disp_al_mod_list_t *new_mod;
	/* struct dsi_init_para dsi_para; */
	struct disp_notifier_block *ptr;

	DE_INF("disp_deinit_al\n");
	/* delete all disp mod exist in this plat into mod_list */
	list_for_each_entry(new_mod, &mod_list.list, list) {
	if (!new_mod) {
			list_del(&new_mod->list);
			kfree(new_mod);
		}
