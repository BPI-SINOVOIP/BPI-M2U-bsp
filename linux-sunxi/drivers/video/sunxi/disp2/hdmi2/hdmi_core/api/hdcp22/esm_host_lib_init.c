							...
							uint32_t image_size)
{
	...
	HDMI_INFO_MSG("code_phys_addr:0x%x data_phys_addr:0x%x data_size:%d\n",
	...
	HDMI_INFO_MSG("\tESM FW_VERSION\t\t= 0x%x\n", data);
	esm->fw_version = data;
	...
	HDMI_INFO_MSG("\tESM DESIGN_ID\t\t= 0x%x\n", data);
	...
	HDMI_INFO_MSG("\tESM PRODUCT_ID\t\t= 0x%x\n", data);
	...
	HDMI_INFO_MSG("\tESM FW_INFO\t\t= 0x%x\n", data);
...
ESM_STATUS esm_get_configuration(esm_instance_t *esm, esm_config_t *returned_config)
{
	...
	case ESM_HDCP_HDMI_TX_ID:
		...
		HDMI_INFO_MSG("\tconfig_version\t\t= %d (TX)", esm->app_id);
		...
		HDMI_INFO_MSG("\tpairing_enabled\t\t= %d (TX)", esm->hdcp_c.c_int.pairing_enabled);
		HDMI_INFO_MSG("\tmax_pairing_devices\t= %d (TX)", esm->hdcp_c.c_int.max_pairing_devices);
		HDMI_INFO_MSG("\tlvc_timeout\t\t= %d ms (TX)", esm->hdcp_c.c_int.tx_lvc_timeout);
		HDMI_INFO_MSG("\tlvc_ignore_timeout\t= %d ms (TX)",
	...
	HDMI_INFO_MSG("\t------------------------------");
	HDMI_INFO_MSG("\texcept_mode\t\t= %x", esm->esm_ctrl.c_int.except_mode);
	HDMI_INFO_MSG("\text_mem_total_size\t= %d", esm->esm_ctrl.c_int.ext_mem_total_size);
	HDMI_INFO_MSG("\text_mem_cfgdata_size\t= %d", esm->esm_ctrl.c_int.ext_mem_cfgdata_size);
	HDMI_INFO_MSG("\text_mem_cpireg_size\t= %d", esm->esm_ctrl.c_int.ext_mem_cpireg_size);
	HDMI_INFO_MSG("\text_mem_mb_size\t\t= %d", esm->esm_ctrl.c_int.ext_mem_mb_size);
	HDMI_INFO_MSG("\text_mem_except_size\t= %d", esm->esm_ctrl.c_int.ext_mem_except_size);
	HDMI_INFO_MSG("\text_mem_logging_size\t= %d", esm->esm_ctrl.c_int.ext_mem_logging_size);
	HDMI_INFO_MSG("\text_mem_srm_size\t= %d", esm->esm_ctrl.c_int.ext_mem_srm_size);
	HDMI_INFO_MSG("\text_mem_topo_size\t= %d", esm->esm_ctrl.c_int.ext_mem_topo_size);
	HDMI_INFO_MSG("\text_mem_topo_seed_size\t= %d", esm->esm_ctrl.c_int.ext_mem_topo_seed_size);
	HDMI_INFO_MSG("\text_mem_pairing_size\t= %d", esm->esm_ctrl.c_int.ext_mem_pairing_size);
	HDMI_INFO_MSG("\twoocompatability\t= %d", esm->hdcp_c.woocompatability);
	HDMI_INFO_MSG("\take_timeouts\t\t= %d", esm->hdcp_c.c_int.ake_timeouts);
	HDMI_INFO_MSG("\ti2c_freq bitrate\t= %d khz", esm->esm_ctrl.i2c_freq);
	HDMI_INFO_MSG("\tcee_enable\t\t= %d", esm->hdcp_c.c_int.cee_enable);
	HDMI_INFO_MSG("\tcee_state_wait_time\t= %d us", esm->esm_ctrl.cee_state_wait_time);

	HDMI_INFO_MSG("\tesm_cpi_off\t\t= %d", esm->esm_cpi_off);
	HDMI_INFO_MSG("\tesm_mb_off\t\t= %d", esm->esm_mb_off);
	HDMI_INFO_MSG("\tesm_exc_off\t\t= %d", esm->esm_exc_off);
	HDMI_INFO_MSG("\tesm_log_(header)_off\t= %d", esm->esm_log_header_off);
	HDMI_INFO_MSG("\tesm_srm_off\t\t= %d", esm->esm_srm_off);
	HDMI_INFO_MSG("\tesm_topo_off\t\t= %d", esm->esm_topo_off);
	HDMI_INFO_MSG("\tesm_topo_slots\t\t= %d", esm->esm_topo_slots);
	HDMI_INFO_MSG("\tesm_topo_seed_off\t= %d", esm->esm_topo_seed_off);
	HDMI_INFO_MSG("\tesm_pair_off\t\t= %d", esm->esm_pair_off);
	HDMI_INFO_MSG("\tesm_pair_size\t\t= %d", esm->esm_pair_size);
	...
	if (!((fw_major >= HOSTLIB_ESM_MAJOR) && (fw_minor >= HOSTLIB_ESM_MINOR) &&
		...
		HDMI_INFO_MSG("ESM FW VERSION MAY NOT WORK WITH HOSTLIB VERSION");
		HDMI_INFO_MSG("ESM FW_VERSION = 0x%x.%x.%x.%x",
	...
	esm_host_driver_t *driver, esm_config_t *config)
{
	...
	HDMI_INFO_MSG("image:%x image_size:%x\n", image, image_size);
	if (esm == 0)
		return ESM_HL_NO_INSTANCE;
