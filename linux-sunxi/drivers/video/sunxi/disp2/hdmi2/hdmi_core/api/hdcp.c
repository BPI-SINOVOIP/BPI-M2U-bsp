...
static void hdcp22_avmute_ovr_enable(hdmi_tx_dev_t *dev, u8 val, u8 enable)
{
	dev_write_mask(dev, HDCP22REG_CTRL1, HDCP22REG_CTRL1_HDCP22_AVMUTE_OVR_EN_MASK, val);
	dev_write_mask(dev, HDCP22REG_CTRL1, HDCP22REG_CTRL1_HDCP22_AVMUTE_OVR_VAL_MASK, enable);
}
...
static int hdcp22_is_supported(hdmi_tx_dev_t *dev)
{
	...
	if (ddc_read_hdcp2Version(dev, &data) == 0) {
		...
		pr_info("ddc_read_hdcp2Version fail\n");
		return -1;
	}
	return -1;
}
...
{
	mc_hdcp_clock_enable(hdmi_dev, enable ? 0 : 1);/*0:enable   1:disable*/
...
{
	LOG_TRACE();
	mc_hdcp_clock_enable(dev, 0);/*0:enable   1:disable*/
	_DisableEncryption(dev, TRUE);

	dev_write_mask(dev, 0x1000C, 1 << 5, 0x0);/*bypass hdcp_block = 0*/
...
static void hdcp22_configure(hdmi_tx_dev_t *dev, hdcpParams_t *hdcp, videoParams_t *video)
{
	...
	if (esm_tx_open() < 0)
		return;
	set_hdcp22_authenticate_and_data_enable();
	hdcp14_enable = false;
	hdcp22_enable = true;
}
...
void hdcp22_initial(hdmi_tx_dev_t *dev, hdcpParams_t *hdcp)
{
	if (esm_tx_initial(hdcp->esm_hpi_base,
			 ...
			 hdcp->esm_data_size))
		pr_info("ERROR: esm_tx_initial failed\n");
}
...
u8 hdcp_event_handler(hdmi_tx_dev_t *dev, int *param, u32 irq_stat)
{
	...
	if (interrupt_status != 0)
		pr_info("hdcp_interrupt_status 0x%x\n", interrupt_status);
	...
	if (interrupt_status & INT_KSV_ACCESS) {
		pr_info("INT_KSV_ACCESS\n");
		return HDCP_IDLE;
	}

	if (interrupt_status & INT_KSV_SHA1_DONE) {
		pr_info("INT_KSV_SHA1_DONE\n");
		if (!_ksv_sha1_status(dev)) {
			pr_info("ksv_sha1_status succeed\n");
			hdcp_sw_reset(dev);
			return HDCP_IDLE;
		} else {
			pr_info("ksv_sha1_status failed\n");
			return HDCP_FAILED;
		}
	}
#endif
