...
int dp_sink_init(u32 sel)
{
	...
	ret = aux_wr(sel, 0x00600, 1,
		     fp_tx_buf); /* set sink to D0 mode(Normal Operation Mode)
	...
	case 0x00:
		training_aux_rd_interval_CR =
		    100; /* Link Status/Adjust Request read interval during CR
			    phase --- 100us */
		training_aux_rd_interval_EQ =
		    400; /* Link Status/Adjust Request read interval during EQ
			    phase --- 400us */
		break;
	case 0x01:
	...
	/* DPTX ENHANCED_FRAME_EN shall be set to 1 when DPRX ENHANCED_FRAME_CAP
	 * is set */
