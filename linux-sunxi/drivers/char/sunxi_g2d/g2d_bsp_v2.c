...
__s32 g2d_irq_query(void)
{
	...
	tmp = read_wvalue(G2D_MIXER_INI);
	if (tmp & 0x1) {
		write_wvalue(G2D_MIXER_INI, tmp);
		return 0;
	}
	return -1;
}
...
__s32 g2d_irq_enable(void)
{
	write_wvalue(G2D_MIXER_INI, 0x10);
	return 0;
}
...
{
	write_wvalue(G2D_MIXER_INI, 0x0);
	return 0;
}
...
__s32 g2d_bsp_bitblt(g2d_image_enh *src, g2d_image_enh *dst, __u32 flag)
{
	...
	if (G2D_BLT_NONE == (flag & 0x0fffffff)) {
		...
		write_wvalue(ROT_IFMT, src->format & 0x1F);
		...
		pitch1 = cal_align(ucnt * src->width, src->align[1]);
		write_wvalue(ROT_IPITCH1, pitch1);
		pitch2 = cal_align(vcnt * src->width, src->align[2]);
		...
		pitch1 = cal_align(ucnt * dst->width, dst->align[1]);
		write_wvalue(ROT_OPITCH1, pitch1);
		pitch2 = cal_align(vcnt * dst->width, dst->align[2]);
