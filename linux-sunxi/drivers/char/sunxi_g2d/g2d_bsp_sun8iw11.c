...
__u64 mixer_get_addr(__u32 buffer_addr, __u32 format, __u32 stride, __u32 x,
		     __u32 y)
{
	__u32 bpp = 0;
	__u64 addr = 0;
	...
	if (para->dst_rect.w > 0x400) {
		...
		for (i = 0; i < cnt; i++) {
			...
			if ((para->flag & G2D_BLT_ROTATE90)
				...
				write_wvalue(G2D_SCALER_SIZE_REG,
					     (para->dst_rect.h -
					      1) | ((0x400 - 1) << 16));
				reg_val =
				    (para->src_rect.w / para->dst_rect.h) << 16;
				...
				reg_val =
				    (para->src_rect.h / para->dst_rect.h) << 16;
					...
					if (para->dst_image.format ==
						...
						addr_val =
						    ...
						    (__u64) ((para->dst_image.
							     ...
							     bppnum);
					reg_val =
						read_wvalue(G2D_DMA_HADDR_REG);
					/* high addr in bits */
					reg_val |=
						((addr_val >> 32) & 0xF) << 16;
					write_wvalue(G2D_DMA_HADDR_REG,
						     reg_val);
					/* low addr in bits */
					reg_val = addr_val & 0xFFFFFFFF;
					write_wvalue(G2D_DMA2_LADDR_REG,
						     reg_val);
					write_wvalue(G2D_DMA2_STRIDE_REG,
						     para->dst_image.w *
						     bppnum);
					write_wvalue(G2D_DMA2_SIZE_REG,
						     (0x400 -
						      1) | ((para->dst_rect.h -
							     1) << 16));
					reg_val =
					    read_wvalue(G2D_DMA2_CONTROL_REG);
					reg_val |= (5 << 8) | G2D_IDMA_ENABLE;
					write_wvalue(G2D_DMA2_CONTROL_REG,
						     reg_val);
				}
			...
			if ((para->flag & G2D_BLT_ROTATE90)) {
				...
				scaler_outy = para->dst_rect.y;
				scaler_outx =
				    para->dst_rect.x - para->dst_image.w;
			} else {
				scaler_outy = para->dst_rect.y;
				scaler_outx += 0x400;
			}
...
__u32 mixer_micro_block_set(g2d_blt *para)
{
	...
	if (para->src_image.format > 0x20) {
		...
		if (para->src_image.format > 0x23
			...
			reg_val = 0x20 << 17;
		write_wvalue(G2D_DMA0_CONTROL_REG,
			     (reg_val |
			      (read_wvalue(G2D_DMA0_CONTROL_REG) &
			       0xff81ffff)));
		write_wvalue(G2D_DMA1_CONTROL_REG,
			     (reg_val |
			      (read_wvalue(G2D_DMA1_CONTROL_REG) &
			       0xff81ffff)));
		write_wvalue(G2D_DMA0_MBCTL_REG,
			     (para->src_rect.y & 0xFFFF) << 16 | (para->
								  src_rect.
								  x & 0xFFFF));
