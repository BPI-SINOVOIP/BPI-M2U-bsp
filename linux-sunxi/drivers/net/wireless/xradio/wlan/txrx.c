...
#endif /*ROAM_OFFLOAD*/
        /*for scanResult.timestamp in Android5.0(more advanced version) Framework(WiFi) */
        struct timespec ts;
        get_monotonic_boottime(&ts);
        do_div(ts.tv_nsec, 1000);
        if (ieee80211_is_beacon(mgmt->frame_control) && !arg->status ){
                mgmt->u.beacon.timestamp = ((u64)ts.tv_sec * 1000000 + ts.tv_nsec);
        }
        else if(ieee80211_is_probe_resp(mgmt->frame_control) && !arg->status){
