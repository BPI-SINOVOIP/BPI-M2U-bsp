...
module_param_string(file_path, file_path, sizeof(file_path), S_IRUGO | S_IWUSR);
...
static void sample_save_frame(struct work_struct *work)
{
	...
	fp = filp_open(file_path, O_RDWR | O_APPEND | O_CREAT, 0666);
	if (IS_ERR_OR_NULL(fp)) {
