				  ...
				  const char *name, int index, int *gpiop)
{
	...
	struct gpio_config of_flags;

	if (of_find_property(node, name, NULL)) {
		gpio = of_get_named_gpio_flags(node, name, index, (enum of_gpio_flags *)&of_flags);
		if (gpio == -ENOENT)
			return 0;
		...
		flags = (of_flags.data & OF_GPIO_ACTIVE_LOW) ? GPIOF_OUT_INIT_LOW :
							GPIOF_OUT_INIT_HIGH;
